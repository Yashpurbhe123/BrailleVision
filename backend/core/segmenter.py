...
class BrailleCellSegmenter:
    ...
    def separate_cells_horizontally(self, word_image: np.ndarray, cells: list[dict]) -> list[dict]:
        """
        Separate individual cells from word image with MINIMAL preprocessing.
        
        Key: Extract cells as-is, let classifier handle preprocessing.
        """
        separated_cells = []
        sorted_cells = sorted(cells, key=lambda c: c.get('x', 0))
        
        logger.info(f"[WORD-DECODE] Separating {len(sorted_cells)} cells (minimal preprocessing)")
        ...
        for idx, cell in enumerate(sorted_cells):
            ...
            # Extract cell with SMALL padding only
            padding = 2  # minimal padding
            x1_padded = int(max(0, round(x1 - padding)))
            y1_padded = int(max(0, round(y1 - padding)))
            x2_padded = int(min(word_image.shape[1], round(x2 + padding)))
            y2_padded = int(min(word_image.shape[0], round(y2 + padding)))
            ...
            # Ensure square aspect ratio (pad with white)
            ...
            separated_cell = {
                ...
                'x': cell.get('x'),
                'y': cell.get('y'),
                'dot_count': cell.get('dot_count'),
                'pattern': cell.get('pattern'),
            }
            
            separated_cells.append(separated_cell)
            logger.info(f"  Cell {idx}: Extracted {cell_image.shape} → {max_dim}x{max_dim}")
